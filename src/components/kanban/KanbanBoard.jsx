      ...
      {selectedLeadForModal && (() => {
        ...
        return (
          <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center z-50 p-4 animate-fade-in">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/10 rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
              
              {/* Header Modal */}
              <div className="p-5 border-b border-slate-100 dark:border-white/5 flex items-center justify-between bg-slate-50/50 dark:bg-dark-900/50">
                <div className="flex items-center space-x-2">
                  <Sparkles className="w-4 h-4 text-emerald-500" />
                  <h3 className="text-xs font-black text-slate-800 dark:text-white uppercase tracking-wider">
                    Ficha Comercial 360° - Liliana León CRM
                  </h3>
                </div>
                <button
                  onClick={() => setSelectedLeadForModal(null)}
                  className="p-1.5 rounded-xl hover:bg-slate-200 dark:hover:bg-white/10 text-slate-400 hover:text-slate-700 dark:hover:text-white transition-all cursor-pointer"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto space-y-5 flex-1">
                ...
                {/* Header Prospecto */}
                <div className="flex items-center space-x-4 bg-slate-50 dark:bg-slate-950/40 p-4 rounded-2xl border border-slate-100 dark:border-white/5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-[11px] text-emerald-600 dark:text-emerald-400 font-extrabold mt-1">
                      Asesor Asignado: {lead.assigned_user_name || 'Asesor Principal ANCLA'}
                    </p>
                  </div>
                </div>

                {/* Grid de Edición de Campos 360° */}
                <div className="grid grid-cols-2 gap-4">
                  
                  {/* Cambiar Etapa del Embudo */}
                  <div>
                    <label className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider block mb-1">Etapa del Embudo</label>
                    <select
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          ...
                          alert(err.message || "Error al cambiar de etapa.");
                        }
                      }}
                      className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-xl px-3 py-2 text-xs font-bold text-slate-800 dark:text-white focus:outline-none cursor-pointer"
                    >
                      {stages.map(st => (
                  ...
                  {/* Editar Presupuesto Estimado ($COP) */}
                  <div>
                    <label className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider block mb-1">Presupuesto Estimado ($COP)</label>
                    <div className="flex items-center space-x-1">
                      <input
                        ...
                        onBlur={async () => {
                          ...
                          if (!isNaN(val)) {
                            await updateLeadDetails(lead.id, { estimated_budget: val });
                            setSelectedLeadForModal(prev => ({ ...prev, estimated_budget: val }));
                          }
                        }}
                        placeholder="Ej. 250000000"
                        className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-xl px-3 py-2 text-xs font-bold text-emerald-600 dark:text-emerald-400 focus:outline-none"
                      />
                    </div>
                  </div>
                  ...
                  {/* Editar Proyecto */}
                  <div>
                    <label className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider block mb-1">Tipo de Proyecto</label>
                    <select
                      value={lead.interest_product || 'Flex Home'}
                      onChange={async (e) => {
                        const val = e.target.value;
                        await updateLeadDetails(lead.id, { interest_product: val });
                        setSelectedLeadForModal(prev => ({ ...prev, interest_product: val }));
                      }}
                      className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-xl px-3 py-2 text-xs font-semibold text-slate-800 dark:text-white focus:outline-none cursor-pointer"
                    >
                      <option value="Flex Home">Flex Home</option>
                      <option value="Living">Living</option>
                      <option value="Llave en Mano">Llave en Mano</option>
                  ...
                  {/* Editar Ciudad del Lote */}
                  <div>
                    <label className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider block mb-1">Ciudad del Lote</label>
                    <select
                      value={lead.lot_city || 'Armenia'}
                      onChange={async (e) => {
                        const val = e.target.value;
                        await updateLeadDetails(lead.id, { lot_city: val });
                        setSelectedLeadForModal(prev => ({ ...prev, lot_city: val }));
                      }}
                      className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-white/5 rounded-xl px-3 py-2 text-xs font-semibold text-slate-800 dark:text-white focus:outline-none cursor-pointer"
                    >
                ...
                {/* Kill Switch Piloto Automático IA */}
                <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-950/30 border border-slate-200 dark:border-white/5">
                  <div>
                    <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block">Piloto Automático Sofía IA</span>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block mt-0.5">Respuestas inteligentes y pre-calificación activa</span>
                  </div>
                  <button
                    type="button"
                    onClick={async () => {
                ...
                <div className="space-y-3 pt-2 border-t border-slate-100 dark:border-white/5">
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 uppercase font-black tracking-wider block">📝 Bitácora & Notas Privadas de Asesor</span>
                  
                  <div className="space-y-2 max-h-[160px] overflow-y-auto pr-1">
                    {loadingNotes ? (
                      <p className="text-xs text-slate-400 italic">Cargando notas...</p>
                    ) : internalNotes.length === 0 ? (
                      <p className="text-[11px] text-slate-400 dark:text-slate-500 italic p-3 bg-slate-50 dark:bg-slate-950/20 border border-slate-100 dark:border-white/5 rounded-xl">
                        No hay comentarios guardados para este cliente.
                      </p>
                    ) : (
                      internalNotes.map((note) => (
                        <div key={note.id} className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-2xl text-[11px] text-slate-800 dark:text-slate-200 space-y-1">
                          <div className="flex items-center justify-between text-[9px] font-bold text-amber-600 dark:text-amber-400">
                            <span>✍️ Nota Interna</span>
                  ...
                  <form onSubmit={handleAddNote} className="space-y-2">
                    <textarea
                      ...
                      placeholder="Escribe una nota comercial sobre el cliente o cita..."
                      rows="2"
                    ...
                    <div className="flex justify-end">
                      <button
                        ...
                        className="px-4 py-2 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-dark-950 font-bold rounded-xl text-[11px] cursor-pointer shadow-sm transition-all"
                      >
              ...
              {/* Footer Acciones */}
              <div className="p-5 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-slate-950/40 flex space-x-3">
                <button
                  type="button"
                  onClick={() => {
                    fetchMessages(lead.id);
                  ...
                  className="flex-1 bg-emerald-500 hover:bg-emerald-400 text-dark-950 font-black py-2.5 px-4 rounded-xl text-xs cursor-pointer shadow-md transition-all flex items-center justify-center space-x-2"
                >
                  <MessageCircle className="w-4 h-4" />
                  <span>💬 Ventana de WhatsApp</span>
                </button>
                <button
                  type="button"
                  onClick={() => setSelectedLeadForModal(null)}
                  className="py-2.5 px-5 bg-slate-200 hover:bg-slate-300 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-xl text-xs cursor-pointer transition-all"
                >
                  Cerrar
                </button>
              </div>
